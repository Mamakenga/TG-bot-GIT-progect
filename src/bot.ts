...
this.bot.onText(/\/testfinal$/, this.handleTestFinal.bind(this));
...
// ✅ НОВАЯ КОМАНДА: Тестирование финального сообщения и теста персонализации
private async handleTestFinal(msg: TelegramBot.Message): Promise<void> {
  const telegramId = msg.from?.id;
  const chatId = msg.chat.id;
  ...
  try {
    ...
    await this.bot.sendMessage(chatId, 
      `🧪 **ТЕСТ ФИНАЛЬНОГО СООБЩЕНИЯ**\n\n` +
      `Симулирую завершение курса и показываю финальное сообщение с тестом персонализации...\n\n`
    );

    // Симулируем финальное сообщение как при завершении 7-го дня
    setTimeout(async () => {
      try {
        const finalMessage = 
        ...
        await this.bot.sendMessage(chatId, 
          `✅ **ТЕСТ ГОТОВ!**\n\n` +
          `🔹 Нажми "🔍 Узнать свой путь дальше" - для прохождения теста\n` +
          `🔹 Нажми "📱 Связаться с психологом" - для показа контактов\n\n` +
          `💡 **Доступные команды:**\n` +
          `• /testfinal - повторить тест финального сообщения\n` +
          `• /test - тест текущего дня\n` +
          `• /nextday - перейти к следующему дню`
        );

      } catch (error) {
        console.error('Ошибка в testfinal timeout:', error);
      }
    }, 1500);

  } catch (error) {
    console.error('❌ Ошибка в handleTestFinal:', error);
    await this.bot.sendMessage(chatId, 'Произошла ошибка при тестировании финального сообщения');
  }
}
